...
USERNAME=xplodwild
BRANCH=android-4.4
GERRIT=gerrit.axxionrom.org
GITHUB_ORG=axxionrom
...
git push ssh://gerrit.axxionrom.org:29418/$repo_name $BRANCH
...
if [ $? != 0 ]; then
	...
	if [ "$forcepush" = "y" ]; then
		git push -f ssh://gerrit.axxionrom.org:29418/$repo_name $BRANCH
	fi
fi
